...
function(limit = 5) {
  ...
  if (!can$ok) {
    return(list(status="error",
                message=sprintf("Mouse TPM file not found at: %s", DEFAULT_MOUSE_TPM)))
  }
...
function() {
  mk <- DEFAULT_MARKERS
  mk$Gene  <- toupper(trimws(mk$Gene))
  mk$Class <- toupper(trimws(mk$Class))
  
  out <- lapply(seq_len(nrow(mk)), function(i) {
    list(
      value = mk$Gene[i],
      label = sprintf("%s (%s)", mk$Gene[i], mk$Class[i]),
      class = mk$Class[i]
    )
  })
  
  list(status = "ok", markers = out)
}


# ---------- JSON UPLOAD (preferred path from Flask/React) ----------
# 
# Also honors ?marker=IL6 in the query string.
#* @post /transpca_json
#* @serializer unboxedJSON
function(req, res) {
  ...
  # marker can come from JSON or query (?marker=)
  marker_in  <- parsed$marker %||% req$args$marker %||% NULL
  ...
  tryCatch({
    run_transpca_core(
      ...
      marker     = parsed$marker %||% req$args$marker %||% NULL  # <- important
...
# ---------- ABSOLUTE-PATH POST (curl-friendly local testing) ----------
# Fields:
#   file_path=/ABS/PATH/matrix.txt
#   meta_path=/ABS/PATH/meta.txt     (optional)
#   marker=IL6                       (optional)
#   size_col=Day  color_col=...  shape_col=...  label_col=...
#* @post /transpca
#* @serializer unboxedJSON
function(file_path = "", meta_path = "", size_col = "", color_col = "", shape_col = "", label_col = "", marker = "") {
  if (!nzchar(file_path)) {
    return(list(
      status  = "error",
      message = paste(
        "Provide absolute paths (no uploads). Example:",
        "  curl -sS http://127.0.0.1:5050/transpca \\",
        "    -d file_path=/ABS/PATH/YourData.txt \\",
        "    -d meta_path=/ABS/PATH/YourMeta.txt \\",
        "    -d marker=IL1B",
        sep = "\n"
      )
    ))
